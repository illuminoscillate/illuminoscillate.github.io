 ...
 function __jQueryYouTubeChannelReceiveData(data) {
     ...
     $.each(data.feed.entry, function(i, e) {
         if (cnt < __opts.numberToDisplay) {
             ...
             var out = '<li class="video"><a href="' + 
                  e.link[0].href + '" title="' + e.title.$t + '"><img src="http://i.ytimg.com/vi/' + 
                  videoId + '/2.jpg"/><span>' + e.title.$t + '</span>';
             if (!__opts.hideAuthor) {
                 out = out + '<p>Author: ' + e.author[0].name.$t + '</p>';
             }
             out = out + '</div>';
    ...
    $.fn.youTubeChannel = function(options) {
        ...
        return this.each(function() {
            if (__opts.userName != null) {
                videoDiv.append("<ul id=\"channel_list\"></div>");
                __mainDiv = $("#channel_list");
                __mainDiv.hide();
